...
return (
    <Link href={`/admin/${page}/${action}/${item._id}`} className='flex justify-between gap-10 mdl:items-center text-md font-medium p-2 hover:bg-accentBg/25'>
        <div className='flex flex-col gap-1'>
            <span className='text-accentBg font-bold text-base leading-5 mdl:text-xl'>{item.displayName}</span>
            ...
            <div className='hidden lg:flex gap-2 text-gray-600 font-semibold flex-grow -mt-2'>
                {item.username}
            </div>
            <div className='hidden lg:flex gap-2 text-gray-600 font-semibold flex-grow -mt-2'>
                {item.email}
           ...
           <span className='font-medium text-sm whitespace-nowrap flex-shrink-0'>{DateConverter({ mongoDate: item.createdAt })}</span>
    </Link>
    )
}
