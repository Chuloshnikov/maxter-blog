...
const Statistics = ({
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await axios.put(`/api/posts/${catSlug}/${postId}/like`, {
        userId,
      });
  ...
  const handleDislike = async () => {
    ...
    try {
      const response = await axios.put(
        `/api/posts/${catSlug}/${postId}/dislike`,
        { userId }
      );
      setLikes(response.data.likes);
      setDislikes(response.data.dislikes);
    ...
    <div className="flex gap-2 items-center text-sm text-gray-600 font-semibold">
      ...
      <div>
        <div
          ...
          className={`flex gap-1 items-center cursor-pointer ${
            userReaction === "like" ? "text-accentBg" : ""
          }`}
        >
          {likes}
      ...
      <div>
        <div
          ...
          className={`flex gap-1 items-center cursor-pointer ${
            userReaction === "dislike" ? "text-accentBg" : ""
          }`}
        >
          {dislikes}
