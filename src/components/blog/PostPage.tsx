...
const PostPage = ({ post }: { post: PostInfo }) => {
  return (
    <div className=" flex flex-col xl:flex-row justify-evenly mx-2">
      <div className="flex flex-col gap-4 mx-auto w-full xl:max-w-5xl mb-4 xl:mb-0">
        <div className="flex flex-col gap-1">
          ...
          <h2 className="text-2xl text-accentBg font-bold">{post.title}</h2>
          ...
          <p className="text-lg break-words overflow-hidden">{post.desc}</p>
        </div>
        <div className="flex items-end justify-end">
          <Statistics
            ...
            initialDislikes={Number(post.dislikes)}
            catSlug={post.catSlug}
          />
        </div>
        <div>
          <CommentCreatorForm id={String(post._id)} />
          <CommentsContainer id={String(post._id)} />
        </div>
      </div>
